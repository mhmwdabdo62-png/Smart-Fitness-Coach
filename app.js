...
document.getElementById('save-diet-btn').addEventListener('click', () => {
    ...
    set(ref(db, `users/${currentUser.uid}/dietTableHTML`), tableHTML).then(() => alert("تم الحفظ!"));
});

document.getElementById('reset-diet-btn').addEventListener('click', () => {
    if(confirm("حذف الجدول؟")) {
...
document.getElementById('save-workout-btn').addEventListener('click', () => {
    ...
    set(ref(db, `users/${currentUser.uid}/workoutTableHTML`), tableHTML).then(() => alert("تم الحفظ!"));
});
...
document.getElementById('ai-send-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        document.getElementById(loadingId)?.remove();
        aiChatBox.innerHTML += `<p style="color: red; font-size: 13px;">مشكلة تقنية: ${e.message}</p>`;
    }
});
